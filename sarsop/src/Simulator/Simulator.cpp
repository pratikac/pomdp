...
int main(int argc, char **argv) 
{
	...
	cout << "\nSimulating ..." << endl;
        ...
        vector< vector<int> > state_trajectories;
        ...
        for (int currSim = 0; currSim < p->simNum; currSim++) 
        {
            ...
            state_trajectories.push_back(curr_traj);
            rewards.push_back(expReward);

            rewardCollector.addEntry(currSim, reward, expReward);
            rewardCollector.printReward(currSim);

        }
        ofstream traj_out("state_trajectories.dat");
        for(unsigned int t1 = 0; t1 < state_trajectories.size(); t1++)
        {
        	traj_out << t1 << "\t" << rewards[t1] << endl;
        	vector<int> tmp = state_trajectories[t1];
            for(unsigned int t2 = 0; t2 < tmp.size(); t2++)
            {
            	traj_out << tmp[t2] << "\t";
            }
            traj_out << endl;
