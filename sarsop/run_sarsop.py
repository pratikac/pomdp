...
if args.solve:
    os.system("src/pomdpsol --timeout "+args.timeout+" -p 1e-2 -o "+policy_file+" "+prob_file)
...
if args.test:
    num_vert = [10, 12, 14]
    discount = [0.96]
    process_noise = 0.01*linspace(5,15,11)
    for vert in num_vert:
        for d in discount:
            for noise in process_noise:
                os.system("../main "+str(vert)+" "+str(d)+" "+str(noise))
                os.system("src/pomdpsol --timeout 10 -p 1e-2 -o "+policy_file+" "+prob_file)
                os.system("src/pomdpsim --simLen="+args.simlen+" --simNum="+args.simnum+" --policy-file "+policy_file+" "+prob_file)
